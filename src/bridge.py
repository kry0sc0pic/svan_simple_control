...
try:
    ...
    command_publisher = rospy.Publisher(
        "/svan/simple_control", SvanCommand, queue_size=1
    )
    ros_connected = True
...
class OperationModeRequest(BaseModel):
    operation_mode: int = Field(
        ..., description="Operation mode (1=STOP, 2=TWIRL, 3=PUSHUP, 4=TROT, 5=SLEEP)"
    )
...
class HeightRequest(BaseModel):
    height: int = Field(..., description="Height state (1=UP, 2=DOWN, 3=STOP)")
...
async def root():
    ...
    if not ros_connected:
        message = (
            "SVAN Control Bridge is running in MOCK MODE (ROS not connected). "
            + message
        )
...
async def set_operation_mode(request: OperationModeRequest):
    ...
    if ros_connected:
        try:
            ...
            return {
                "status": "ok",
                "message": f"Set operation mode to {request.operation_mode}",
            }
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Failed to set operation mode: {str(e)}"
            )
    else:
        return {
            "status": "mock",
            "message": f"Set operation mode to {request.operation_mode} (MOCK MODE)",
        }
...
async def set_movement(request: MovementRequest):
    ...
    if ros_connected:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to set movement: {str(e)}"
            )
    else:
        return {
            "status": "mock",
...
async def set_roll(request: RollRequest):
    ...
    cmd_data = {"command_type": "ROLL", "roll": roll}
    command_history.append(cmd_data)
...
async def set_pitch(request: PitchRequest):
    ...
    cmd_data = {"command_type": "PITCH", "pitch": pitch}
    command_history.append(cmd_data)
    ...
    if ros_connected:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to set pitch: {str(e)}"
            )
    else:
...
async def set_yaw(request: YawRequest):
    cmd_data = {"command_type": "YAW", "yaw": request.yaw}
    command_history.append(cmd_data)
...
async def set_height(request: HeightRequest):
    cmd_data = {"command_type": "HEIGHT", "height": request.height}
    command_history.append(cmd_data)
    ...
    if ros_connected:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to set height: {str(e)}"
            )
    else:
        return {
            "status": "mock",
            "message": f"Set height to {request.height} (MOCK MODE)",
        }
...
if __name__ == "__main__":
    ...
    print(f"View command history at http://0.0.0.0:8888/history")
    if not ros_connected:
